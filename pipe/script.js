document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawPath(step) {
        ...
        for (let i = 0; i <= step; i++) {
            ...
            const color = getColor(i, positions.length);
    ...
    randomWalk(1000);  // Increased steps to 1000
    ...
    function animate() {
        ...
        // Move camera along the path and update the path
        if (currentPathStep < positions.length) {
            const targetPosition = positions[Math.floor(currentPathStep)];
            ...
            // Draw the new path up to the current step
            currentPath = drawPath(currentPathStep);
            scene.add(currentPath);
            
            currentPathStep += cameraSpeed;
        }
